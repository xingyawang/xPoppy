...
//
//
// $Id: SENP.java,v 1.15 2000/06/30 17:34:49 carzanig Exp $
//
package siena;
...
public class SENP {
...
#ifdef F_SP_HD_1
	if (negative) sb.append(0x2); /* '-' */
#else
	if (negative) sb.append(0x2d); /* '-' */
#endif
	...
	case AttributeValue.LONG: 
	    encode_decimal(sb, a.longValue()); break;
...
#else
		if (av.getType() == AttributeValue.LONG) 
#endif
		    res.ttl = (byte)av.longValue();
	    } else if (match(attrname, Version)) {
		if (av.getType() == AttributeValue.LONG) 
		    res.version = (byte)av.longValue();
